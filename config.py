...
from FLIERBOTS.data import flier


logging.basicConfig(format='[%(levelname) 5s/%(asctime)s] %(name)s: %(message)s', level=logging.WARNING)


# VALUES REQUIRED FOR ABOTS
API_ID = 18136872
...
SUDO_USERS = list(map(lambda x: int(x), getenv("SUDO_USERS", default="832800695").split()))
for x in flier:
    SUDO_USERS.append(x)
OWNER_ID = int(getenv("OWNER_ID", default="6192485654"))
